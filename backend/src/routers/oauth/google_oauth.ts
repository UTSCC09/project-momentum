...
oauthGoogleRouter.get("/googlecallback", async (req, res) => {
  ...
  if (!code) {
    res.redirect("https://localhost:5173/?success=false&error=No authorization code provided");
    return;
  }
  ...
  try {
    ...
    if (!id_token) {
      res.redirect("https://localhost:5173/?success=false&error=No ID token received");
      return;
    }
    ...
    if (!kid) {
      res.redirect("https://localhost:5173/?success=false&error=Invalid token payload");
      return;
    }
    ...
    if (googleOauth) {
      ...
      res.redirect(`https://localhost:5173/?success=true&id=${googleOauth.User.id}`);
      return;
    }
    ...
    res.redirect(`https://localhost:5173/?success=true&id=${user.id}`);
  } catch (error) {
    console.error("Error during Google callback:", error);
    res.redirect(`https://localhost:5173/?success=false&error=${encodeURIComponent(error as string)}`);
  }
});
